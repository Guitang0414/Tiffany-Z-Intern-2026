...
| Category name → id resolve | **Agent** | Agent 端启动时一次性 cache `/items/categories` 映射,POST 时直接送 `category_id`。这样 POST body 跟 schema 完全对齐,Directus hook 不用做这一步 |
...
flowchart TD
    ...
    A3 -->|POST /items/articles<br/>(已含 category_id,<br/>Agent 启动时 cache 过)<br/>Directus API Token 认证| D[⚙️ Directus]
    D -->|beforeCreate hook:<br/>final_* = ai_*<br/>(一次性 INSERT)| DB[(💾 PostgreSQL)]
```
...
>
> **MVP 阶段的关键约束**:
> - **claim / unclaim 机制不实现**:编辑直接打开文章编辑。MVP 依赖 **Directus Revisions** 留痕,具体并发冲突机制(Data Studio 是否自动 409 / 还是 last-write-wins)**spike 时确认**;如内置不足,Phase 2 加自定义 optimistic check 或 claim / lock
...
> 实线 = 必经路径,虚线 = 可选分支。**并发冲突**:MVP 依赖 Directus Revisions 留痕(可在 Revisions tab 回看 / restore);Data Studio 自带的并发检测行为 spike 时确认,如不足 Phase 2 加自定义 409 检查。

##### Editor 在 Data Studio 里的视图

> Directus Data Studio **自动渲染 admin UI**,**不需要写自定义前端代码**(自定义 button / interface 等少量代码除外)。下面是要在 Directus 里配的内容(side bar 显示、字段权限、列表 preset、看板)。
...
- **Concurrent edits**:**MVP 依赖 Directus Revisions 留痕**;Data Studio 是否自动做 409 冲突检测 **spike 时验证**;若不足,Phase 2 加自定义 optimistic check 或 claim / lock
