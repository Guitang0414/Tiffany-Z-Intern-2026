...
**每篇文章对应一个 category**(MVP 简化,不做多对多)。Agent 拿到 AI 判定的 name 后,**自己在本地 cache 里查到 `category_id`**(启动时一次性 GET `/items/categories` 缓存全表),最后 POST 时**只送 `category_id`**(跟 articles schema 完全对齐)。
...
| `reviewed_by` | Publish 自定义 action(在 Directus 配置)/ 或 `beforeUpdate` lifecycle hook | 点 Publish 时,**custom action 或 hook 显式写入** `reviewed_by = currentUser.id`(Directus 不自动记录,要主动赋值) |
...
- `articles.beforeUpdate`:
  ...
  - **当 `status` 转为 `PUBLISHING`,自动写入 `reviewed_by = currentUser.id`**(Directus 不会自动记,必须 hook 显式写)
